...
const WHITE_ICON_TECHS = new Set<string>([
  'Next.js',
  'NextAuth',
  'Radix UI'
]);

const getTechIconUrl = (techName: string): string => {
  const slug = TECH_ICON_SLUGS[techName]
    || techName.toLowerCase().replace(/\+/g, 'plus').replace(/\./g, 'dot').replace(/\s*\/\s*/g, '').replace(/[^a-z0-9]/g, '');

  const colorSuffix = WHITE_ICON_TECHS.has(techName) ? '/ffffff' : '';
                ...
                >
                  <img
                    ...
                    onError={(event) => {
                      ...
                      const fallbackColorSuffix = WHITE_ICON_TECHS.has(tech) ? '/ffffff' : '';
                      event.currentTarget.src = `https://cdn.simpleicons.org/simpleicons${fallbackColorSuffix}`;
                    }}
                  />
